# Faits vérifiés — cordistes (source unique des chiffres du blog)

> Les deux agents rédacteurs DOIVENT tirer leurs chiffres d'ici. Aucun chiffre
> inventé ni "de mémoire". Si un chiffre manque, `WebSearch` d'abord, puis
> ajoute-le ici avec sa date de vérification. Dernière révision : **2026-09-03**.

## ⚠️ Corrections vs anciennes versions des agents (chiffres FAUX à ne plus utiliser)

| Ancien chiffre (FAUX) | Réalité 2026 |
|---|---|
| CQP TPS N1 « ≈ 3 500-5 500 €, 3 semaines » | **5 000-8 000 €**, **350-420 h (~10 semaines)**, période en entreprise incluse |
| Validité CQP « recyclage triennal » | **Validité 5 ans**, **recyclage MAC obligatoire tous les 4 ans** |
| CPF « gratuit » | **Reste à charge obligatoire de 150 €** depuis le 02/04/2026 (sauf demandeur d'emploi / cofinancement) |

## Formation

- **CQP1 (cordiste niveau initial / TPS N1)** : 350-420 h (~10 semaines dont stage entreprise). **5 000-8 000 €** selon le centre.
- **CQP2 (confirmé)** : ~2-3 semaines, ~2 500-4 000 €.
- **IRATA L1** : 1 semaine (40 h) + examen. **~1 200-1 800 €**. Reconnaissance internationale.
- **IRATA L2 / L3** : exige des heures loguées (≥1 000 h et 12 mois entre niveaux). L3 = superviseur.
- **Recyclage / MAC** : CQP tous les 4 ans (~600-1 200 €) ; IRATA revalidation tous les 3 ans.
- **Financements** : CPF (reste à charge 150 € au 02/04/2026), **AIF France Travail** (demandeurs d'emploi, souvent 100 % + AREF), **OPCO Constructys** (salariés BTP, Pro-A / abondement), plan de développement des compétences employeur.

## Rémunération (2026)

- **Salarié débutant** : 1 400-1 550 € net/mois (légèrement au-dessus du SMIC).
- **Salarié confirmé** : 1 700-1 900 € net/mois, plus selon spécialisation.
- **Intérim / ETT** : primes de précarité + déplacements ; TJ facturé à l'agence supérieur mais net variable.
- **Indépendant — TJM facturé** : 280-450 €/j junior · 450-650 €/j confirmé · 650-900 €/j expert/chantier complexe.
- Spécialisations mieux payées : **nucléaire, éolien (pales), offshore, pétrochimie**.

## Statut & gestion (indépendant)

- **Charges micro-entreprise** : ≈ 22 % du CA (BNC prestations de services + CFP).
- **RC pro cordiste** : 400-900 €/an selon CA. La RC pro BTP standard **n'inclut pas** l'accès sur cordes → vérifier la mention explicite TAC.
- **Cotisation SFETH** : ~250-400 €/an pour un indépendant.
- **Kit EPI complet débutant** : 1 500-2 500 € (harnais cuissard, longes double, descendeur autobloquant, bloqueurs ventral+poignée, casque, connecteurs, sangles). **Durée de vie textile ≈ 10 ans** (dès fabrication), **contrôle EPI obligatoire au moins tous les 12 mois** par personne compétente.

## Marché & secteurs (contexte 2026, pour actualité et angles)

- Métier **en tension** : 140+ offres actives en continu sur les jobboards, listé parmi les métiers qui recrutent.
- **Éolien** : parc terrestre > 23 GW fin 2024 ; offshore commercial en service (**Saint-Nazaire, Fécamp, Saint-Brieuc**). Forte demande de techniciens pales — certif **GWO** souvent exigée en plus d'IRATA.
- **Nucléaire** : recrutements actifs sur CNPE (ex. Cruas) ; habilitations spécifiques (CSQ, RP) en plus des cordes.
- **Télécom** : antennes, pylônes, déploiement fibre/5G.
- **Bâtiment/industrie** : ravalement, étanchéité, inspection d'ouvrages d'art, maintenance industrielle.

## Terminologie officielle (à employer, pas de jargon gratuit)

- Organismes : **SFETH** (syndicat), **IRATA** (international), **OPPBTP** (prévention BTP), **CARSAT**, **INRS**, **France Travail**, **Constructys** (OPCO BTP), **Qualiopi** (certif qualité OF).
- Métier : accès sur cordes, TAC (travaux d'accès difficile), technicien cordiste, EPI, point d'ancrage, anti-chute mobile, CATEC (espaces confinés), plan de prévention, PPSPS, double protection (deux cordes).
- Statuts : micro-entreprise, EURL, SASU, portage salarial, ETT (intérim), sous-traitance / renfort.

## Règles factuelles à ne jamais enfreindre

- Ne jamais promettre de revenus garantis. Toujours des fourchettes + "selon zone/réseau/spécialité".
- Reconnaître honnêtement : saisonnalité, premiers mois durs, isolement de l'indépendant, pénibilité/TMS, dépendance aux donneurs d'ordre.
- Distinguer clairement salariat / intérim / indépendance — ne pas supposer que le lecteur est indépendant.
- Toute affirmation réglementaire chiffrée doit être vérifiable (année, source). En cas de doute : `WebSearch` puis mise à jour de ce fichier.
