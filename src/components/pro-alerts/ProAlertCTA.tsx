...
export const ProAlertCTA: React.FC<Props> = ({ defaultDepartments, defaultEmail }) => {
    ...
    // ─── Form body (réutilisé desktop inline + mobile sheet) ────────────────
    const formBody = (
    ...
    // ─── DESKTOP / TABLETTE (md+) — bloc inline inchangé ────────────────────
    const desktopBlock = done ? (
        <div className="hidden md:flex items-start sm:items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl p-4 sm:px-5 sm:py-3.5 mb-6">
        ...
        <div className="hidden md:block bg-gradient-to-r from-brand-blue/5 to-brand-blue-light/5 border border-brand-blue/20 rounded-xl mb-6 overflow-hidden">
            <div className="flex items-center justify-between gap-4 px-5 py-3.5">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-blue via-brand-blue-light to-brand-blue animate-gradient-shift flex items-center justify-center flex-shrink-0 shadow-sm">
                ...
                <button
                    ...
                    className="flex-shrink-0 inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-gradient-to-r from-brand-blue via-brand-blue-light to-brand-blue animate-gradient-shift text-white rounded-lg text-xs font-bold uppercase tracking-wide shadow-sm hover:shadow-md transition-shadow"
                >
    ...
    // ─── MOBILE (<md) — FAB sticky en bas + bottom-sheet ────────────────────
    const mobileFAB = (
        <div className="md:hidden">
            {/* FAB sticky bottom — toujours présent, état done = bouton vert */}
            <div className="fixed bottom-3 left-3 right-3 z-40 pointer-events-none">
                <button
                    type="button"
                    onClick={() => !done && setExpanded(true)}
                    disabled={done}
                    className={`pointer-events-auto w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold uppercase tracking-wide shadow-lg ${
                        done
                            ? 'bg-emerald-600 text-white shadow-emerald-600/30 cursor-default'
                            : 'bg-gradient-to-r from-brand-blue via-brand-blue-light to-brand-blue animate-gradient-shift text-white shadow-brand-blue/30'
                    }`}
                    aria-label={done ? 'Alertes activées' : 'Activer mes alertes'}
                >
                    {done ? (
                        <>
                            <Check size={16} />
            ...
            {/* Bottom sheet — formulaire en plein écran */}
            <AnimatePresence>
                {expanded && (
                    <>
                        <motion.div
                        ...
                        <motion.div
                            ...
                            className="fixed left-0 right-0 bottom-0 z-50 bg-white rounded-t-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
                        >
                            <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-slate-100 px-4 py-3 flex items-center justify-between">
                                <div className="flex items-center gap-2 min-w-0">
                                    <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-brand-blue via-brand-blue-light to-brand-blue animate-gradient-shift flex items-center justify-center flex-shrink-0">
                                        <Bell size={14} className="text-white" />
                                    </div>
                                    <h3 className="text-sm font-bold text-slate-900 truncate">
                                        Activer mes alertes missions
                                    </h3>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => setExpanded(false)}
                                    aria-label="Fermer"
                                    className="flex-shrink-0 p-1.5 rounded-md text-slate-500 hover:bg-slate-100"
                                >
                                    <X size={18} />
                                </button>
                            </div>
