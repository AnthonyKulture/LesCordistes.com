...
export const CitySEOPage: React.FC<Props> = ({ citySlug: propCitySlug }) => {
    ...
    return (
        <div className="min-h-screen bg-white">
            <Helmet>
                <title>Cordiste {name} : Travaux en Hauteur & Accès Difficiles | LesCordistes</title>
                <meta name="description" content={`Trouvez un cordiste qualifié à ${name} pour vos travaux en accès difficile. Devis gratuit sous 48h. Experts certifiés CQP/IRATA.`} />
                <meta name="geo.region" content={`${codeISO}-${department}`} />
                <meta name="geo.placename" content={name} />
                <meta name="geo.position" content={`${lat};${lng}`} />
                <link rel="canonical" href={`https://lescordistes.com/cordiste-${citySlug}`} />
            ...
            <div className="container max-w-5xl py-12">
                ...
                <div className="prose prose-lg max-w-none text-slate-700 mb-16">
                    ...
                    <h3>Key Takeaways (Les avantages de l'accès sur cordes)</h3>
                    <ul>
                            ...
                            <div key={clusterType} className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                ...
                                <ul className="space-y-3">
                                    {SEO_SERVICES.filter(s => s.cluster === clusterType).map(service => (
                                        <li key={service.id}>
                                            <Link 
                                                to={`/cordiste-${citySlug}/${service.slug}`}
